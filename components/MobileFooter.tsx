                ...
                <ul className='flex flex-row justify-between px-6'>
                    {NAV_ITEMS.map(({href, icon}) => {
                        ...
                        return (
                            <li key={href}>
                                <Link href={href}
                                      className={`hover:text-blue-300 text-gray-500 p-4 text-sm transition-colors ${isActive ? 'primary border-b-2 border-blue-600' : ''}`}>
                                    <FontAwesomeIcon icon={icon} className='text-lg mt-2'/>
                                </Link>
                            </li>
                        )})}
                </ul>
